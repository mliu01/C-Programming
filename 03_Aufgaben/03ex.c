...
int sum_of_doubly_divisibles(int n, int l, int m) {
    ...
    while (n_counter < n) {
        ...
        if (is_multiple_of(number, l)&& is_multiple_of(number, m)) {
            result = result + number;
            n_counter++;
        }
        number++;
...
Canvas descending_diagonal(Canvas c) {
    ...
    for(int y=canvas_height(c)-1; y>=0; y--) { // man fängt oben links bei canvas_height-1 an

        for (int x=0; x<canvas_height(c)-y && x<canvas_width(c); x++) { // gibt vor, wie viele X-Punkte aufwärts in einer Reihe gefärbt werden sollen, 
            canvas_set_black(c, x, y);
        }
    }
    return c;
...
Canvas draw_rectangle(Canvas c, int x, int y, int width, int height) {
    ...
    for (int j=y; j>y-height; j--) {
        for (int i=x; i<x+width; i++) {
            ...
            if ((j<canvas_height(c) && j>=0) && (i<canvas_width(c)&&i>=0)) {
